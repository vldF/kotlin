...
object LeakingThisChecker : FirClassChecker() {
    override fun check(declaration: FirClass<*>, context: CheckerContext, reporter: DiagnosticReporter) {
        ...
        val data = CfaVisitorWithFunctionsNesting(properties, functions).getData(classGraph)
        GraphReporterVisitor(data, properties, reporter).processGraph(classGraph)
    }

    private class CfaVisitorWithFunctionsNesting(
        ...
        override fun visitNode(node: CFGNode<*>, data: MutableMap<CFGNode<*>, PathAwarePropertyUsageInfo>): PathAwarePropertyUsageInfo {
        ...
        override fun visitVariableAssignmentNode(
            ...
            return dataForNode.update(symbol) { it + EventOccurrencesRange.EXACTLY_ONCE }
        }
        ...
        override fun visitQualifiedAccessNode(
            ...
            return data
                .filterKeys { it is VariableAssignmentNode }
                .mapNotNull { (k, v) ->
        ...
        private fun isAccessNodeMayBeProcessed(symbol: FirPropertySymbol, node: QualifiedAccessNode): Boolean {
            ...
            // if (node.owner.declaration is FirProperty) return false
    ...
    private class GraphReporterVisitor(
        ...
        fun processGraph(graph: ControlFlowGraph) {
        ...
        override fun visitQualifiedAccessNode(node: QualifiedAccessNode) {
            ...
            if (dataForNode?.isAlwaysInitialized != true && shouldReport(variableSymbol, node)) {
                val reportTo = node.fir.source
                reporter.report(reportTo, LEAKING_THIS)
            }
        }

        private fun shouldReport(symbol: FirPropertySymbol, node: QualifiedAccessNode): Boolean {
            if (node.owner.declaration is FirAnonymousInitializer) return false
        ...
        private val FirPropertySymbol.itWillBeInitializedLater: Boolean
            get() {
                return data.any { (k, _) -> k is VariableAssignmentNode && k.fir.lValue.resolvedSymbol == this }
            }
    ...
    private val EventOccurrencesRange.isAlwaysInitialized
        get() = this == EventOccurrencesRange.AT_LEAST_ONCE || this == EventOccurrencesRange.EXACTLY_ONCE || this == EventOccurrencesRange.MORE_THAN_ONCE
}